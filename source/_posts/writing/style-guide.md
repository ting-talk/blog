...
在众多小写字母中，突然出现一个大写字母，就能达到强调（鹤立鸡群）的作用，所以专有名字的首字母或者全部字母会是大写。
...
-  I、Ⅱ、Ⅲ（一个整体）：但是比较难出去
-  I、II、III、IIII、V（用拉丁字母 I、V、X 代替）

[The Type — 文字 / 设计 / 文化 » 字谈字畅 059：数，不胜数（III）](https://www.thetype.com/typechat/ep-059/)：探讨一种来自欧洲的传统数字表记法——罗马数字
...
中文标点符号必须使用全角字符。排版时，大部分中文标点符号占用的宽度等于一个中文字符，例如 `。`；而英文一般使用半角标点符号。排版时，它们大约占用的宽度等于半个中文字符，例如 `.`
...
请注意，以上关于「全角/半角」和「一个/半个中文字符」的说法并不严谨，只是一个笼统且简化的描述，并不是对的，详情见以下资料：

- 图文 | [孔雀计划：中文字体排印的思路 -  The Type](https://www.thetype.com/kongque/)
- 图文 | [全角半角碎碎念 - The Type](https://www.thetype.com/2018/02/14211/)
